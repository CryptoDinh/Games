...
function goPage(page){
	...
	switch(page){
		...
		case 'result':
			...
			resultScoreTxt.text = textDisplay.resultScoreText.replace('[NUMBER]', addCommas(playerData.score));
			stopGame();
...
function toggleInstruction(con){
	if(con){
		if(gameSettings.gamePlayType){
			instructionTxt.text = textDisplay.instructionTxt1;
		}else{
			instructionTxt.text = textDisplay.instructionTxt2;
		}
...
function updateStat(){
	if(gameSettings.gamePlayType){
		chanceTxt.text = textDisplay.chancesText.replace('[NUMBER]', playerData.chance);
		creditTxt.text = textDisplay.creditText.replace('[NUMBER]', addCommas(Math.floor(playerData.point)));
	}else{
		creditTxt.text = textDisplay.creditText.replace('[NUMBER]', addCommas(Math.floor(playerData.point)));
		betTxt.text = textDisplay.creditText.replace('[NUMBER]', addCommas(Math.floor(playerData.bet)));	
	}
}
	...
	TweenMax.to(playerData, 1, {overwrite:true, onComplete:function(){
		...
		if(gameSettings.secondWheel){
			...
			if(wheelSegmentType == 1){
				...
				statusTxt.text = textDisplay.statusText_arr[2].replace('[NUMBER]', addCommas(wheelSegmentNumber));
				var speedTween = .5;
				TweenMax.to(itemStatusBg, speedTween, {overwrite:true, onComplete:function(){
					playSound('soundTone');
					statusTxt.text = textDisplay.statusText_arr[2].replace('[NUMBER]', addCommas(wheelSegmentNumber)) +' x '+wheelInnerSegmentNumber;
					...
					TweenMax.to(itemStatusBg, speedTween, {overwrite:true, onComplete:function(){
						...
						statusTxt.text = textDisplay.statusText_arr[3].replace('[NUMBER]', addCommas(winPoint));
						
						if(winPoint > 0){
							//win
							if(wheelSegmentType == 2){
								playSound('soundJackpot');
								statusTxt.text = textDisplay.statusText_arr[5].replace('[NUMBER]', addCommas(winPoint));
							}else{
								playSound('soundWin');	
							}
